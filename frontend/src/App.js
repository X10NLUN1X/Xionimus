...
function App() {
  ...
  const downloadGeneratedCode = async (content, messageId) => {
    try {
      ...
      if (codeBlocks.length === 0) {
        toast.error('Kein Code in dieser Nachricht gefunden');
        return;
      }
      
      // Determine file extension based on code content
      const detectedLanguage = detectCodeLanguage(codeBlocks[0].code);
      ...
      if (codeBlocks.length === 1) {
        ...
        a.href = url;
        a.download = `generated_code_${Date.now()}.${fileExtension}`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        
        toast.success(`Code als .${fileExtension} Datei heruntergeladen!`);
      } else {
        // Multiple files - use backend ZIP creation
        try {
          const response = await fetch(`${API}/download-code-rar`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (response.ok) {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `generated_code_${Date.now()}.zip`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
            
            toast.success('Code als ZIP-Archiv heruntergeladen!');
          } else {
            throw new Error('Backend ZIP creation failed');
          }
        } catch (zipError) {
          console.warn('ZIP creation failed, falling back to text file:', zipError);
          // Fallback: combine all code blocks into one file
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `generated_code_combined_${Date.now()}.txt`;
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
          URL.revokeObjectURL(url);
          
          toast.success('Code als kombinierte Textdatei heruntergeladen!');
        }
      }
      
    } catch (error) {
      console.error('Code download error:', error);
      toast.error('Fehler beim Herunterladen des Codes');
    }
  };
  ...
  const extractCodeFromMessage = (content) => {
    ...
    // If no markdown blocks, try to extract inline code or plain code
    if (codeBlocks.length === 0) {
      // Look for common programming patterns
      if (content.includes('def ') || content.includes('import ') || content.includes('class ')) {
        codeBlocks.push({ language: 'python', code: content });
      } else if (content.includes('function ') || content.includes('const ') || content.includes('let ')) {
        codeBlocks.push({ language: 'javascript', code: content });
      } else if (content.includes('<html') || content.includes('<!DOCTYPE')) {
        codeBlocks.push({ language: 'html', code: content });
      }
    }
  ...
  // Detect programming language from code content (for download functionality)
  const detectCodeLanguage = (code) => {
    if (!code) return 'text';
    
    const codeLines = code.toLowerCase();
    
    if (codeLines.includes('def ') || codeLines.includes('import ') || codeLines.includes('print(')) {
      return 'python';
    } else if (codeLines.includes('function ') || codeLines.includes('const ') || codeLines.includes('console.log')) {
      return 'javascript';
    } else if (codeLines.includes('<html') || codeLines.includes('<!doctype')) {
      return 'html';
    } else if (codeLines.includes('class ') && codeLines.includes('public ')) {
      return 'java';
    } else if (codeLines.includes('#include') || codeLines.includes('int main')) {
      return 'cpp';
    } else if (codeLines.includes('interface ') || codeLines.includes('type ')) {
      return 'typescript';
    }
