...
def get_user_api_keys(db, user_id: str) -> Dict[str, str]:
    ...
    Returns dict with provider -> api_key mapping
    """
    try:
        ...
        for key in api_keys:
            try:
                ...
                logger.warning(f"⚠️ Failed to decrypt {key.provider} key for user {user_id}: {decrypt_error}")
                keys_to_delete.append(key)
        
        # Delete corrupted keys
        for key in keys_to_delete:
            logger.info(f"🗑️ Deleting corrupted {key.provider} key for user {user_id}")
            db.delete(key)
        
        if keys_to_delete:
            db.commit()
            logger.warning(f"⚠️ Deleted {len(keys_to_delete)} corrupted API keys")
        
        logger.info(f"🔑 Retrieved {len(decrypted_keys)} API keys for user {user_id}: {list(decrypted_keys.keys())}")
