...
async def save_api_key(
    ...
    try:
        ...
        api_key = api_key.replace('\u200b', '')  # Remove zero-width space
        api_key = api_key.replace('\ufeff', '')  # Remove BOM
        
        logger.info(f"🔑 Saving API key for {request.provider} (length: {len(api_key)})")
...
async def test_connection(
    ...
    try:
        ...
        # Clean decrypted key (safety measure)
        api_key = api_key.strip() if api_key else ""
        
        logger.info(f"🧪 Testing {request.provider} API key (length: {len(api_key)})")
