...
class SandboxExecutor:
    ...
    LANGUAGE_CONFIGS = {
        ...
        "csharp": {
            "extension": ".cs",
            # Windows uses csc.exe or dotnet, Unix uses mcs/mono
            "compile_command": ["csc" if IS_WINDOWS else "mcs", "-out:"] if not IS_WINDOWS else ["csc", "/out:"],
            "command": ["mono"] if not IS_WINDOWS else [],  # Windows runs .exe directly
            "timeout": 30,
            "memory_limit_mb": 512,
            "compiled": True
        },
    ...
    def __init__(self):
        ...
        if sys.platform == 'win32':
            ...
            # Unix: Use /tmp
            self.workspace_dir = Path("tempfile.gettempdir() + os.sepxionimus_sandbox")
    ...
    def execute_code(
        ...
        try:
            ...
            # Make bash scripts executable (Unix only)
            if language == "bash" and not IS_WINDOWS:
            ...
            if config.get("compiled", False):
                ...
                if language in ["cpp", "c", "go"]:
                    ...
                    # java -Xmx256m -Xms64m -cp <directory> ClassName
                    class_name = compile_result.get("class_name")
                    cmd = config["command"] + ["-cp", str(exec_dir), class_name]
            ...
            if language in ["cpp", "c"]:
                ...
                if IS_WINDOWS:
                    ...
                    if shutil.which("dotnet"):
                        compile_cmd = ["dotnet", "build", "-o", str(exec_dir), str(code_file)]
                    elif shutil.which("csc"):
                        compile_cmd = ["csc", f"/out:{binary_path}", str(code_file)]
                    else:
                        compile_cmd = ["mcs", f"-out:{binary_path}", str(code_file)]  # Fallback
                else:
                ...
                process = subprocess.Popen(compile_cmd, **popen_kwargs)
    ...
    def _execute_with_limits(
        ...
        def set_limits():
            ...
            try:
                ...
                if not (is_java or is_go or is_typescript):
                    ...
                    if HAS_RESOURCE: resource.setrlimit(resource.RLIMIT_AS, (memory_bytes, memory_bytes))
                
                # Set CPU time limit
                if HAS_RESOURCE: resource.setrlimit(resource.RLIMIT_CPU, (timeout, timeout))
                
                # Disable core dumps
                if HAS_RESOURCE: resource.setrlimit(resource.RLIMIT_CORE, (0, 0))
                
                # Limit number of processes
                if HAS_RESOURCE: resource.setrlimit(resource.RLIMIT_NPROC, (50, 50))
            except Exception as e:
        ...
        try:
            ...
            # Add Windows-specific flag to prevent console window
            if IS_WINDOWS:
                popen_kwargs["creationflags"] = subprocess.CREATE_NO_WINDOW
