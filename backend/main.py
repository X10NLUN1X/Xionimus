...
# Import API routes
from app.api import chat, auth, files, workspace, github, testing, agents, supervisor, bulk_files, knowledge, vision, sessions, chat_stream, multimodal_api, rag_api, workspace_api, clipboard_api, edit, tokens, metrics, rate_limits, session_management, github_pat, session_fork, file_upload, version, sandbox, sandbox_templates, api_keys, multi_agents, research_history, health, monitoring, github_admin
...
async def auth_and_rate_limit_middleware(request: Request, call_next):
    ...
    public_paths = {
        ...
        "/api/v1/multi-agents/types",  # Agent types list - public for UI dropdown
        "/api/github/oauth/status",  # GitHub OAuth status - public (no sensitive data)
...
# Health Check Endpoints (Kubernetes-ready, no auth required)
app.include_router(health.router, prefix="/api", tags=["health"])
app.include_router(health.router, prefix="/api/v1", tags=["health", "v1"])

# Monitoring Endpoints (requires auth)
app.include_router(monitoring.router, prefix="/api/v1", tags=["monitoring", "v1"])
...
if os.getenv("ENABLE_GITHUB_INTEGRATION", "true").lower() == "true":
    ...
    app.include_router(github_pat.router, prefix="/api/v1/github", tags=["github-oauth", "v1"])  # OAuth endpoints
    app.include_router(github_admin.router, prefix="/api/v1/github", tags=["github-admin", "v1"])  # Admin endpoints
    # Legacy
    ...
    app.include_router(github_pat.router, prefix="/api/github", tags=["github-oauth", "legacy"])  # OAuth endpoints
